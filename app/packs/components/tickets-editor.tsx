...
export function TicketsEditor({ tickets, onSave, restaurantes, restauranteId }: TicketsEditorProps) {
  ...
  const handleInvalidSubmit = (errors: Record<string, unknown>) => {
    ...
    const ticketsErrors = (errors as { tickets?: unknown }).tickets;
    if (Array.isArray(ticketsErrors)) {
      const idx = ticketsErrors.findIndex(Boolean);
      if (idx >= 0) {
        setFocusedTicketIndex(idx);
  ...
  const removeTicket = (index: number) => {
    const nombre = String(form.getValues(`tickets.${index}.Nombre` as const) ?? '').trim();
    const label = nombre || `Ticket ${index + 1}`;
  ...
  const handleSubmit = async (values: TicketsFormValues) => {
    try {
      ...
      showSaveErrorMessage(err instanceof Error ? err.message : 'No se pudieron guardar los tickets.');
    } finally {
      setIsSaving(false);
    }
  };
