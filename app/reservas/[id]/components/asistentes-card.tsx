...
type Props = {
  ...
  reservaFecha?: string | Date | null;
  reservaEstado?: string | null;
  aforoSolicitadoLabel?: string | null;
  stats: {
...
const formatFileDate = (value: unknown) => {
...
const buildAsistentesRows = (asistencias: AsistenciaDetalle[]): AsistenteRow[] => {
  ...
  asistencias.forEach((asistencia) => {
    ...
    const tienePago = Boolean((asistencia as { paymentIntentId?: string | null }).paymentIntentId);
    const nombre = getAsistenciaNombre(asistencia, asistencia.id);
    rows.push({
      id: asistencia.id,
      nombre,
...
const buildCSV = (
  ...
  const baseHeaders = includePago ? ['Nombre', 'Tipo de asistencia', 'Pago', 'Alergias'] : ['Nombre', 'Tipo de asistencia', 'Alergias'];
  ...
  const pushRow = (
    ...
    const base = includePago ? [nombre, tipoAsistencia, pagoValue, alergiasValue] : [nombre, tipoAsistencia, alergiasValue];
  ...
  const getPagoCell = (tipoLabel: string, tienePago: boolean) => {
    if (!includePago) return '';
    if (tipoLabel !== 'Confirmado') return '—';
    return tienePago ? 'Pagado' : 'Sin pago';
  };
  ...
  asistencias.forEach((asistencia) => {
    ...
    pushRow(nombre, tipoLabel, getPagoCell(tipoLabel, tienePago), alergiasValue, email, telefono, getAsistenciaQuestions(asistencia));
    ...
    invitados?.forEach((inv, index) => {
      ...
      const invQuestions = (inv.questions as Record<string, unknown> | undefined) ?? {};
      pushRow(invNombre, invTipoLabel, getPagoCell(invTipoLabel, tienePago), invAlergias, invEmail, invTelefono, invQuestions);
    });
  });
...
export function AsistentesCard({
  reservaNombre,
  reservaFecha,
  reservaEstado,
  aforoSolicitadoLabel,
  stats,
  alergias,
  preguntas,
  showPaymentStats,
  ...
  const [qaErrors, setQaErrors] = useState<Record<string, { question?: string; options?: string }>>({});
  ...
  const asistentesRowsSorted = useMemo(() => {
    if (!showPaymentStats) return asistentesRows;
    return [...asistentesRows].sort((a, b) => {
      const rank = (row: AsistenteRow) => {
        if (row.tipoAsistencia === 'Confirmado' && !row.tienePago) return 0;
        if (row.tipoAsistencia === 'Confirmado' && row.tienePago) return 1;
        if (row.tipoAsistencia === 'Tal vez') return 2;
        return 3;
      };
      return rank(a) - rank(b);
    });
  }, [asistentesRows, showPaymentStats]);
  const hasPreguntas = preguntas.length > 0;
  const isReservaLockedForQa = ['completado', 'fallado'].includes((reservaEstado ?? '').toLowerCase());
  const apuntadosEntradas = stats.pagados + stats.confirmadosNoPagados;
  const apuntadosValue = showPaymentStats ? apuntadosEntradas : stats.totalAsistentes;

  useEffect(() => {
    if (!isReservaLockedForQa) return;
    if (qaView !== 'editar') return;
    setQaView('respuestas');
  }, [isReservaLockedForQa, qaView]);

  const handleDownloadCSV = () => {
    const csv = buildCSV(alergias, preguntas, !isKomvo, Boolean(showPaymentStats));
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    const userPart = sanitizeFilenamePart(reservaNombre);
    const datePart = formatFileDate(reservaFecha);
    link.download = `asistentes_${userPart}_${datePart}_extra.csv`;
    link.click();
  ...
  const canSaveQa = useMemo(() => validateQuestions(editingQuestions).ok, [editingQuestions]);

  const handleSaveQuestions = async () => {
    if (savingQuestions) return;
    setQaTouched(true);
    const validation = validateQuestions(editingQuestions);
    setQaErrors(validation.errors);
    ...
    return (
      <div className="space-y-4">
        <Card className="gap-3 border-none bg-white p-4 shadow-sm">
          <CardHeader className="space-y-2 p-0">
            ...
            <div className="grid w-full grid-cols-2 gap-2">
                            ...
                            <div>
                              ...
                              {row.esInvitado && (
                                <p className="text-[11px] text-slate-500">
                                  Invitado por {row.invitadoPor ?? 'Usuario principal'}
                                </p>
                              )}
                              <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] font-semibold">
                                ...
                                {showPaymentStats && row.tipoAsistencia === 'Confirmado' ? (
                                  row.tienePago ? (
              ...
              <Button variant="outline" size="sm" onClick={handleDownloadCSV} className="w-full justify-center gap-2">
                <Download className="h-4 w-4" />
                Descargar CSV
              </Button>
            </div>
          </CardHeader>
          <CardContent className="space-y-3 p-0 text-sm text-slate-600">
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <p className="text-xs uppercase tracking-wide text-slate-400">Aforo solicitado</p>
                <p className="text-lg font-semibold text-slate-900">{aforoSolicitadoLabel || '—'}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-wide text-slate-400">Apuntados</p>
                <p className="text-lg font-semibold text-slate-900">{apuntadosValue}</p>
              </div>
            </div>
        ...
        {alergiasList.length > 0 ? (
          <Card className="gap-3 border-none bg-white p-4 shadow-sm">
            <CardHeader className="flex items-start justify-between gap-3 p-0">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Alergias</p>
              </div>
              <Dialog>
                <DialogTrigger asChild>
        ...
        <Card className="gap-3 border-none bg-white p-4 shadow-sm">
          <CardHeader className="flex items-start justify-between gap-3 p-0">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Preguntas</p>
            </div>
            <Dialog
              open={qaDialogOpen}
              onOpenChange={(open) => {
                ...
                if (open) {
                  setQaTouched(false);
                  setQaErrors({});
                  setQaView(hasPreguntas || isReservaLockedForQa ? 'respuestas' : 'editar');
                }
              }}
            >
              <div className="flex items-center gap-2">
                <DialogTrigger asChild>
                  <Button
                    ...
                    onClick={() => setQaView(hasPreguntas || isReservaLockedForQa ? 'respuestas' : 'editar')}
                  >
                    <HelpCircle className="h-4 w-4" />
                    Ver Q&A
                  </Button>
                </DialogTrigger>
                {/* "Ver Q&A" sirve también para añadir/editar cuando no hay preguntas */}
              </div>
              <DialogContent className="max-h-[80vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>Preguntas y respuestas</DialogTitle>
                    <p className="text-sm text-slate-500">
                      Estas serán las preguntas que se le harán a los usuarios cuando compren o marquen asistencia al plan.
                    </p>
                  </DialogHeader>
                  ...
                  {qaView === 'editar' && !isReservaLockedForQa && (
                  ...
                  {qaView === 'respuestas' && (
                    <div className="mt-4 space-y-4">
                      {qaData.length === 0 ? (
                        <p className="text-sm text-slate-500">
                          {hasPreguntas ? 'Aún no hay respuestas de los asistentes.' : 'No hay preguntas configuradas.'}
                        </p>
                      ) : (
                        qaData.map((qa) => (
                            ...
                            <div className="mt-4">
                              <div className="flex items-center justify-between text-xs text-slate-500">
                                ...
                                {qa.respuestasIndividuales.length > 3 && (
                                  ...
                                  >
                                    {expanded[qa.question] ? 'Ver menos' : `Ver todas (${qa.respuestasIndividuales.length})`}
                                  </button>
                                )}
                              </div>
                        ...
                        <div key={item.id} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                          ...
                          <div className="mt-3 grid gap-3 md:grid-cols-2">
	                            <div className="md:col-span-2">
	                              ...
	                              />
                              {qaTouched && qaErrors[item.id]?.question ? (
                                <p className="mt-1 text-xs text-rose-600">{qaErrors[item.id]?.question}</p>
                              ) : null}
	                            </div>
                            <div>
                              ...
                              <select
                                ...
                                onChange={(event) =>
                                  updateQuestion(item.id, {
                                    question_type: event.target.value as QuestionItem['question_type'],
                                  })
                                }
                              >
                            ...
                            <div className="flex items-center gap-2 pt-6">
                              ...
                              {qaTouched && qaErrors[item.id]?.options ? (
                                <p className="text-xs text-rose-600">{qaErrors[item.id]?.options}</p>
                              ) : null}
	                            </div>
	                          )}
                        </div>
                  ...
                  <DialogFooter>
                    ...
                    {qaView === 'editar' && !isReservaLockedForQa && (
                      <Button
                        ...
                        disabled={savingQuestions || !canSaveQa}
                        onClick={async () => {
  ...
  return (
    <Card className="gap-3 border-none bg-white p-4 shadow-sm">
      <CardHeader className="flex items-start justify-between gap-3 p-0 sm:flex-row sm:items-center">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Asistentes</p>
